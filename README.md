...
*Editable Mermaid source in [`docs/flowchart.md`](docs/flowchart.md); regenerate the PNG with `python make_flowchart.py`.*
...
├── notebooks/
│   ├── model_pipeline.ipynb    # end-to-end construction walkthrough (executed, with figures)
...
├── docs/
│   ├── presentation_script.md  # ~5-min bilingual talk script
│   └── flowchart.md            # editable Mermaid flowchart
...
The five required functions live in `recipe_recommender.py`: `train_lda`,
...
python run_demo.py              # train + recommend
# or, as a library:
...
> regenerable). Run `python prepare_data.py` then `python run_demo.py` once to generate them —
> the notebooks load `models/lda_model.pkl`. The committed notebooks already have their outputs
> and figures embedded, so they render on GitHub without running anything.
...
# CLI: python recipe_recommender.py --ingredients ... --diet vegan --diversity 0.5 --top-n 8
```
