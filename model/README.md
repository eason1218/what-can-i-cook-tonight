...
*Editable Mermaid source in [`flowchart.md`](flowchart.md); regenerate the PNG with `python make_flowchart.py`.*
...
├── prepare_data.py                  # download Food.com  ->  recipes_clean.csv
├── run_demo.py                      # end-to-end demo    ->  results.json (+ trains the model)
├── visualize.py                     # Bayesian figures   ->  fig1–fig4 PNGs
├── make_flowchart.py                # pipeline diagram   ->  pipeline_flowchart.png
...
├── recipes_clean.csv                # generated by prepare_data.py  (gitignored)
└── lda_model.pkl                    # trained by run_demo.py        (gitignored)
```
...
> **Note.** `recipes_clean.csv` and `lda_model.pkl` are **gitignored** (large /
> regenerable). Run `python prepare_data.py` then `python run_demo.py` once to generate them —
> the notebooks load `lda_model.pkl`. The committed notebooks already have their outputs
...
Built from the cached posterior (`lda_model.pkl`) — no retraining, runs in seconds.

**Model selection — why K is chosen out-of-sample**

![Model selection: held-out predictive vs in-sample WAIC/LOO across K](fig1_model_selection.png)
...
![Per-topic top ingredients with 94% credible intervals on phi](fig2_topic_phi_posterior.png)

*The "fully Bayesian" view: we keep posterior distributions over φ, not point estimates.*

**Step 3 — the user's flavor posterior**

![P(topic | pantry) with 94% credible intervals](fig3_user_topic_posterior.png)
...
![Top-5 flavor-alignment posterior](fig4_recommendation_uncertainty.png)
