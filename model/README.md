...
LDA treats the corpus as generated by a small set of latent **flavor topics**. With `M` recipes,
`K` topics, and a vocabulary of `V` ingredients:

- each **topic** `k` is a distribution over ingredients `φ_k ~ Dir(β)` → stacked into `φ`, shape `(K, V)`;
- each **recipe** `m` is a mixture of topics `θ_m ~ Dir(α)`, shape `(K,)`;
- each ingredient slot in recipe `m` draws a topic `z ~ Cat(θ_m)`, then an ingredient `w ~ Cat(φ_z)`.

We use **sparse symmetric Dirichlet priors** — `α = 0.1` (a recipe leans on few topics) and
`β = 0.01` (a topic leans on few ingredients) — so the recovered topics are crisp and readable.
Inference is **variational** (scikit-learn's batch VB), not MCMC: that is the deliberate scale
trade-off detailed in Step 1.

| symbol | shape | meaning |
|--------|-------|---------|
| `φ` (`phi`) | `(K, V)` | topic → ingredient distributions. `phi_mean` = point estimate `φ̂`; `phi_samples` = `(B, K, V)` Bootstrap draws |
| `θ` (`theta`) | `(M, K)` | recipe → topic mixtures; its corpus mean is the topic prior `P(topic)` |
| `α`, `β` | scalar | symmetric Dirichlet priors on `θ`, `φ` (`0.1`, `0.01`) |
| `K`, `V` | — | number of topics (default `4`) and vocabulary size (top `500` ingredients by document frequency) |
| `B`, `m` | — | Bootstrap refits (`50`) and resample size (m-out-of-n, `m ≈ 10k`) |
| `U`, `R` | sets | the user's and a recipe's normalized ingredient sets (for *coverage* and *overlap*) |
...
2. **Bootstrap pseudo-posterior.** Refit `B=50` times on resampled recipes (m-out-of-n,
   ...
   *Warm-start mechanics:* each refit seeds sklearn's topic-word variational parameter at
   `λ = φ̂·m + β`, refreshes the cached `exp(E[log φ])` (a digamma update), then runs `n_warm_iter`
   online `partial_fit` passes on the resample — so every draw starts at the consensus `φ̂` and
   moves only as far as its resample pulls it. That is *why* the spread is small and honestly
   reported as stability rather than a posterior width.
...
The product over ingredients assumes they are **conditionally independent given the topic** (a
naive-Bayes likelihood), and `P(topic=k)` is the empirical corpus topic marginal (the mean recipe
mixture `θ̄`). We evaluate this for every Bootstrap sample of `φ` and keep the per-sample posteriors,
so the "flavor profile" carries uncertainty rather than collapsing to a point estimate. This is the
